...
def inspect_metadata():
    ...
    try:
        if os.path.exists(db.db_path):
            ...
            print(
                "No vector database file found. Make sure you've processed documents first."
            )
            return
    ...
    if not db.metadata:
        print(
            "No metadata found in database. Make sure you've processed documents first."
        )
        return
    ...
    for i, chunk in enumerate(db.metadata[:5]):  # Show first 5 chunks
        ...
        if contextual:
            print(
                f"  Contextual Info: {contextual[:200]}{'...' if len(contextual) > 200 else ''}"
            )
