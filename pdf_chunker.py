...
class PDFClusterSemanticChunker:
    def __init__(
        self, gemini_model="gemini-embedding-001", max_chunk_size=400, min_chunk_size=50
    ):
        ...
        for item in text_items:
            ...
            if self._token_count(text) <= self.min_chunk_size:
                ...
                for word in words:
                    ...
                    if (
                        self._token_count(" ".join(current_chunk))
                        >= self.min_chunk_size
                    ):
                        result.append({"text": " ".join(current_chunk), "page": page})
                        current_chunk = []
    ...
    def _get_embeddings_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        result = self.client.models.embed_content(
            ...
            config=types.EmbedContentConfig(
                output_dimensionality=768, task_type="QUESTION_ANSWERING"
            ),
        )
...
def main():
    ...
    chunker = PDFClusterSemanticChunker(max_chunk_size=400, min_chunk_size=50)
