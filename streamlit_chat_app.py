...
class MultiDocumentRAGSystem:
    def __init__(self):
        ...
        self.chunker = PDFClusterSemanticChunker(max_chunk_size=400, min_chunk_size=50)
        self.vector_db = None
        self.es_bm25 = None
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                ...
                try:
                    dataset_entry = self.chunker.create_dataset_from_pdf(
                        temp_path, doc_id=Path(uploaded_file.name).stem
                    )
    ...
    def search_documents(self, query: str, k: int = 10, use_reranking: bool = True):
        ...
        results, _semantic_count, _bm25_count = retrieve_advanced(
            query,
            self.vector_db,
            self.es_bm25,
...
Answer:"""
        ...
        try:
            ...
            return f"Sorry, I encountered an error generating the response: {e!s}"
    ...
    def process_answer_with_citations(self, answer: str) -> str:
        ...
        def replace_grouped_sources(match):
            ...
            for source_num in source_numbers:
                links.append(
                    f'<a href="#source-{source_num}" style="text-decoration: none; color: #1f77b4; font-weight: bold;">[{source_num}]</a>'
                )
        ...
        answer = re.sub(
            r"[\[\(]Source\s+\d+(?:,\s*(?:Source\s+\d+|p\.\s*\d+|\d+))*(?:;\s*Source\s+\d+(?:,\s*(?:Source\s+\d+|p\.\s*\d+|\d+))*)*[\]\)]",
            replace_grouped_sources,
            answer,
        )
...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_files:
            ...
            if has_existing_data:
                st.warning(
                    "⚠️ You have existing processed documents. Processing new files will replace them."
                )
                replace_confirm = st.checkbox(
                    "I understand - replace existing documents", value=False
                )
                ...
                with st.spinner("Processing documents..."):
                    try:
                        processed_files, total_docs = (
                            st.session_state.rag_system.process_uploaded_files(
                        ...
                        st.error(f"Error processing documents: {e!s}")
                    finally:
                        progress_bar.empty()
                        status_text.empty()
            ...
            if st.button(
                ...
                with st.spinner("Deleting all data..."):
                    try:
                        ...
                        st.error(f"Error deleting data: {e!s}")
        ...
        use_reranking = st.checkbox(
            "Use Reranking",
            value=True,
            help="Apply neural reranking for better results",
        )
        num_results = st.slider(
            "Results to retrieve", min_value=3, max_value=25, value=8
        )
    ...
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            if message["role"] == "assistant":
                ...
                processed_content = (
                    st.session_state.rag_system.process_answer_with_citations(
                        message["content"]
                    )
                )
    ...
    if prompt := st.chat_input("Ask a question about your documents..."):
        ...
        if (
            ...
            or not st.session_state.rag_system.vector_db
        ):
        ...
        with (
            ...
            st.spinner("Searching documents and generating response..."),
        ):
            try:
                # Search documents
                search_results = st.session_state.rag_system.search_documents(
                    prompt, k=num_results, use_reranking=use_reranking
                )
                ...
                processed_response = (
                    st.session_state.rag_system.process_answer_with_citations(response)
                )
                ...
                with st.expander("🔍 Retrieved Context", expanded=False):
                    for i, result in enumerate(search_results):
                        ...
                        # Extract structured metadata from contextualized_content
                        contextualized_content = chunk_data.get(
                        ...
                        st.markdown(
                            f'<a name="source-{i + 1}"></a>', unsafe_allow_html=True
                        )
                ...
                st.error(f"Error generating response: {e!s}")
