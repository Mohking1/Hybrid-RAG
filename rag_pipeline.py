...
class ContextualVectorDB:
    ...
    def summarize_pdf(self, pdf_path: str, prompt: str = "Summarize this document") -> str:
    ...
    def generate_chunk_metadata(self, pdf_path: str, chunk_content: str, page_info: str) -> str:
        file_path = pathlib.Path(pdf_path)
        prompt = f"""
        ...
        def process_chunk(doc, chunk):
            ...
            if page_start is not None and page_end is not None:
                if page_start == page_end:
                    ...
                    page_info = f"Pages: {page_start}-{page_end} (chunk spans multiple pages)"
            ...
            if pdf_path:
                context = self.generate_chunk_metadata(pdf_path, chunk['content'], page_info)
            else:
...
class ElasticsearchBM25:
    def __init__(self, index_name: str = "contextual_bm25_index", es_config: Dict[str, Any] = None):
...
def rerank_with_m3(query: str, candidate_chunks: List[Dict[str, Any]], k: int) -> List[Dict[str, Any]]:
    ...
    reranked = [{
        "chunk": candidate_chunks[i]['metadata'],
...
def create_multi_document_dataset(pdf_paths: List[str], chunker=None) -> List[Dict[str, Any]]:
...
def answer_question(query: str, contextual_db, es_bm25, gemini_client=None, k: int = 5, use_reranking: bool = True):
...
Answer:"""
    ...
    try:
        ...
        return f"Error generating response: {str(e)}", search_results
...
def main():
    ...
    answer, results = answer_question(query, contextual_db, es_bm25)
